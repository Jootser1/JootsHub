...
export const ChatSocketHandler = ({ children, conversationId }: ChatSocketHandlerProps) => {
  ...
  useEffect(() => {
    ...
    if (connectedConversations.has(conversationId)) {
      logger.info(`ChatSocketHandler: La conversation ${conversationId} est déjà gérée par une autre instance`);
      return;
    }
    
    logger.debug(`[JOIN_ATTEMPT] Conversation=${conversationId}, isChatConnected=${isChatConnected}, hasJoined=${hasJoinedRef.current}, isActive=${isConversationActive(conversationId)}, stack=${new Error().stack}`);
